...
function TripDetailsPageContent() {
  ...
  const handleForceClose = async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to force close trip"
      );
    } finally {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {trip.status === "ACTIVE" && (
            <>
              <Button onClick={() => setActiveTab("live")} icon={MapIcon}>
                View Live Tracking
              </Button>
              <Button
                variant="outline"
                  ...
                  {trip.progress && (
                    <>
                      ...
                      <div>
                        ...
                        <dd className="mt-1 text-sm font-semibold text-gray-900">
                          {trip.progress.currentPointIndex + 1} of{" "}
                          {trip.points.length}
                        </dd>
                      </div>
                    </>
                      ...
                      {trip.statusHistory.map((history) => (
                        <tr key={history.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <StatusBadge
                              status={history.previousStatus as any}
                              size="sm"
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <StatusBadge
                              status={history.newStatus as any}
                              size="sm"
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {history.deduction !== undefined &&
                            history.deduction !== null &&
                            history.deduction > 0 ? (
                      ...
                      <tr key={check.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {check.distanceToFirstPoint
                            ? `${(check.distanceToFirstPoint / 1000).toFixed(
                                2
                              )} km`
                            : "N/A"}
                        </td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Are you sure you want to force close this trip? This action
                will:
              </p>
              <ul className="list-disc list-inside text-sm text-gray-600 mb-4 space-y-1">
                <li>
                  Change the trip status to <strong>Force Closed</strong>
                </li>
                <li>
                  Deduct <strong>$100</strong> from the trip price (Captain will
                  receive{" "}
                  <strong>${((trip.price ?? 0) - 100).toFixed(2)}</strong>{" "}
                  instead of ${(trip.price ?? 0).toFixed(2)})
                </li>
