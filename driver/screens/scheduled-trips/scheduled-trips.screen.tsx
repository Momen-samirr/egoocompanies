...
export default function ScheduledTripsScreen() {
  ...
  const handleStatusChange = async () => {
    try {
      ...
      Toast.show(error.response?.data?.message || "Failed to update status", {
        type: "danger",
      });
    }
  };
  ...
  const handleStartTrip = async (trip: ScheduledTrip) => {
    try {
      ...
      if (response.data.success) {
        ...
        await AsyncStorage.setItem("activeTripId", trip.id);
      ...
      Toast.show(error.response?.data?.message || "Failed to start trip", {
        type: "danger",
      });
    } finally {
  ...
  const getActivationMessage = (item: ScheduledTrip): string => {
    ...
    if (item.activationStatus.isTooEarly) {
      return `Too early. Earliest start: ${new Date(
        item.activationStatus.earliestStartTime || item.scheduledTime
    ...
    if (
      ...
      return `Too far from first checkpoint (${(
        item.activationStatus.distanceToFirstPoint / 1000
      ).toFixed(1)} km away)`;
    }
  ...
  const renderTripItem = ({ item }: { item: ScheduledTrip }) => {
    ...
    const canStart =
      isOnline &&
      item.activationStatus?.canActivate &&
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {item.points?.length || 0} checkpoint
        ...
        {item.activationStatus && item.status === "SCHEDULED" && (
          ...
          >
            ...
            >
              <Text style={{ fontSize: 16 }}>{canStart ? "✓" : "⏳"}</Text>
              <Text
                style={{
                  color: canStart
                    ? color.semantic.success
            ...
            {isOnline &&
              ...
              item.activationStatus.distanceToFirstPoint <= 5000 && (
                ...
                >
                  {(
                    (item.activationStatus.distanceToFirstPoint || 0) / 1000
                  ).toFixed(2)}{" "}
                  km from first checkpoint
                </Text>
              )}
          </View>
        )}
