...
// New architecture types not available in RN 0.74.3
...
// Stub types for new architecture (not available in RN 0.74.3)
// These must be public because they're used in public inline functions
public enum class LogicalEdge {
  ALL, LEFT, RIGHT, TOP, BOTTOM, START, END, BLOCK, BLOCK_END, BLOCK_START;
  companion object {
    fun fromSpacingType(spacing: Int): LogicalEdge = ALL
  }
}

public enum class BorderRadiusProp {
  ALL, TOP_LEFT, TOP_RIGHT, BOTTOM_RIGHT, BOTTOM_LEFT, TOP_START, TOP_END, BOTTOM_START, BOTTOM_END, END_END, END_START, START_END, START_START
}

public enum class BorderStyle {
  SOLID, DASHED, DOTTED;
  companion object {
    fun fromString(s: String): BorderStyle = SOLID
  }
}

public class LengthPercentage(val value: Float, val type: LengthPercentageType) {
  companion object {
    val POINT = LengthPercentageType.POINT
  }
}
...
public class BoxShadow {
  companion object {
    fun parse(map: Any, context: Any): BoxShadow? = null
  }
}

public object BackgroundStyleApplicator {
  fun setBorderColor(view: View, edge: LogicalEdge, color: Int?) {}
  fun setBorderWidth(view: View, edge: LogicalEdge, width: Float) {}
  fun setBorderRadius(view: View, border: BorderRadiusProp, radius: LengthPercentage?) {}
  fun setBorderStyle(view: View, style: BorderStyle?) {}
  fun setBackgroundColor(view: View, color: Int?) {}
  fun setBoxShadow(view: View, shadows: List<BoxShadow>) {}
}
...
inline fun <reified T : View> ViewDefinitionBuilder<T>.UseBoxShadowProp(crossinline body: (view: T, shadow: List<BoxShadow>) -> Unit) {
  // BOX_SHADOW not available in RN 0.74.3, using a custom prop name
  Prop("boxShadow") { view: T, shadows: ReadableArray? ->
    if (shadows == null) {
      body(view, emptyList())
      return@Prop
    }
