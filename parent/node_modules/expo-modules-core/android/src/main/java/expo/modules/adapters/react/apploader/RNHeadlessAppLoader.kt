    ...
    if (context.applicationContext is ReactApplication) {
      if (!appRecords.containsKey(params.appScopeKey)) {
        ...
        if (BuildConfig.IS_NEW_ARCHITECTURE_ENABLED) {
          // New architecture - API not available in RN 0.74.3, fallback to old architecture
          ...
          // reactHost.addReactInstanceEventListener API doesn't exist in RN 0.74.3
          // Falling through to old architecture implementation
        }
        if (!BuildConfig.IS_NEW_ARCHITECTURE_ENABLED) {
          // Old architecture
  ...
  override fun invalidateApp(appScopeKey: String?): Boolean {
    return if (appRecords.containsKey(appScopeKey) && appRecords[appScopeKey] != null) {
      ...
      if (BuildConfig.IS_NEW_ARCHITECTURE_ENABLED) {
        ...
        // reactHost.destroy API doesn't exist in RN 0.74.3
        // Falling through to old architecture implementation
      }
      if (!BuildConfig.IS_NEW_ARCHITECTURE_ENABLED) {
        // Old architecture
