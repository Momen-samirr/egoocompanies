...
internal fun Project.applyDefaultPlugins() {
  // Don't apply library plugin if application plugin is already applied
...
internal fun Project.applyKotlin(kotlinVersion: String, kspVersion: String) {
  extensions.extraProperties.set("kotlinVersion", kotlinVersion)
  extensions.extraProperties.set("kspVersion", kspVersion)
...
internal fun Project.applyDefaultDependencies() {
  // Only apply dependencies if this is a library module
  if (plugins.hasPlugin("com.android.library")) {
    val modulesCore = rootProject.findProject(":expo-modules-core")
    if (modulesCore != null && project != modulesCore) {
...
internal fun Project.applyDefaultAndroidSdkVersions() {
  // Only apply if this is a library module, not an application module
  if (plugins.hasPlugin("com.android.library")) {
    with(androidLibraryExtension()) {
      applySDKVersions(
        compileSdk = rootProject.extensions.extraProperties.safeGet<Int>("compileSdkVersion")
          ?: logger.warnIfNotDefined("compileSdkVersion", 36),
        minSdk = rootProject.extensions.extraProperties.safeGet<Int>("minSdkVersion")
          ?: logger.warnIfNotDefined("minSdkVersion", 24),
        targetSdk = rootProject.extensions.extraProperties.safeGet<Int>("targetSdkVersion")
...
internal fun Project.applyPublishing(expoModulesExtension: ExpoModuleExtension) {
  // Only apply publishing if this is a library module
  if (!plugins.hasPlugin("com.android.library")) {
